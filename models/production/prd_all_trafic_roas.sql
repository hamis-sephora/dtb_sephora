{{
  config(
    materialized = 'table',
    labels = {'type': 'funnel_google_analytics_cbs', 'contains_pie': 'no', 'category':'reporting'}  
  )
}}
...
from ga_data
left join consolidation_media
on ga_data.id_channel = consolidation_media.id_channel