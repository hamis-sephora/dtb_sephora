...
SELECT
        EXTRACT (WEEK(SUNDAY) from PARSE_DATE('%Y%m%d', date)) AS week_number,
...
cs_raw as ( 
  SELECT
   ...
   CASE
    ...
    GROUP BY Transaction_Date_Local,country,device_cs,week_number 
) , 
...
--) Merge cbs data with google analytics data to have sessions and sessions retrevied 
  ...
  ga.sessions, 
  ga.transactions,
  SUM(ga.transactions) OVER (PARTITION BY ga.week_number, ga.country) total_transactions,
  round(ga.transactions/ SUM(ga.transactions) OVER (PARTITION BY ga.week_number, ga.country),4) as poids_transactions_total,
  cs_final.transactions as cs_transactions,
  round( cs_final.transactions*(round(ga.transactions/ SUM(ga.transactions) OVER (PARTITION BY ga.week_number, ga.country),3)),0) as cs_transactions_pondere, 
  round( ga.transactions/ga.sessions,3) as conv_rate,
   FROM ga
   left join  cs_final
   on ga.week_number=cs_final.week_number
