...
SELECT
       ...
       WHEN REGEXP_CONTAINS(h.page.hostname, r'^(www)\.sephora\.(sa)$') THEN 'me'
        ...
        count (distinct case when h.eCommerceAction.action_type = '2'then CONCAT(fullVisitorId, CAST(visitStartTime AS STRING))	end ) as product_view, 
        ...
        count (distinct case when h.eCommerceAction.action_type = '5'then CONCAT(fullVisitorId, CAST(visitStartTime AS STRING))	end ) as checkout,
        count (distinct case when h.eCommerceAction.action_type = '6'then CONCAT(fullVisitorId, CAST(visitStartTime AS STRING))	end ) as purchase,                 
